...
def init(root):
    ...
    code.append("DIGIT buffer[" + str(root.num_digits_n*2 + root.num_digits_j - root.num_digits_nminusj) + "];")
    return code
...
def calculateLeftOperands(node):
    ...
    if(node.operandSource == "fgsum"):
        # f_sum
        code.extend(scalarprod(node.num_digits_n + node.num_digits_j, "temp2", node.num_digits_j, "p_00+" + str(p_off), "p_01+" + str(p_off), node.num_digits_n, "f_sum+" + str(fg_off), "g_sum+" + str(fg_off)))
        code.append(print_pol("temp2", "f_sum", node.num_digits_j+node.num_digits_n))
        ...
        # g_sum
        code.extend(scalarprod(node.num_digits_n + node.num_digits_j, "temp2", node.num_digits_j, "p_10+" + str(p_off), "p_11+" + str(p_off), node.num_digits_n, "f_sum+" + str(fg_off), "g_sum+" + str(fg_off)))
        code.append(print_pol("temp2", "g_sum", node.num_digits_j+node.num_digits_n))
        code.append(digit_shift(node.num_digits_n+node.num_digits_j, "temp2", node.j))
        ...
        # f_sum
        code.extend(scalarprod(node.num_digits_n + node.num_digits_j, "temp2", node.num_digits_j, "p_00+" + str(p_off), "p_01+" + str(p_off), node.num_digits_n, "f+" + str(fg_off), "g+" + str(fg_off)))
        code.append(print_pol("temp2", "f_sum", node.num_digits_j+node.num_digits_n))
        code.append(digit_shift(node.num_digits_n+node.num_digits_j, "temp2", node.j))
        code.append(memcpy("f_sum+"+str(k.fgsum_offset[node.depth]), "temp2+" + str(node.num_digits_n + node.num_digits_j - node.num_digits_nminusj), node.num_digits_n))
        
        # g_sum
        code.extend(scalarprod(node.num_digits_n + node.num_digits_j, "temp2", node.num_digits_j, "p_10+" + str(p_off), "p_11+" + str(p_off), node.num_digits_n, "f+" + str(fg_off), "g+" + str(fg_off)))
        code.append(print_pol("temp2", "g_sum", node.num_digits_j+node.num_digits_n))
        code.append(digit_shift(node.num_digits_n+node.num_digits_j, "temp2", node.j))
        code.append(memcpy("g_sum+"+str(k.fgsum_offset[node.depth]), "temp2+" + str(node.num_digits_n + node.num_digits_j - node.num_digits_nminusj), node.num_digits_n))
...
def recombine(node):
    ...
    code = []
    code.append("\n// Recombining results: n:" + str(node.n) +", depth: " + str(node.depth))
